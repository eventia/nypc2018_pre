"""
승리팀 찾기
우성이는 카트라이더를 즐기는 유저이다. 우성이는 항상 친구가 많기
때문에 개인전 보다는 팀전을 즐겨 한다. 게임의 종류와 플레이어들의
도착시간이 주어졌을 때, 어느 팀이 이겼는지를 계산하는 프로그램을
만들어 보자.

카트라이더는 아이템전과 스피드전이 있다. 팀은 레드 팀과 블루
팀이 있으며, 문제 편의상 항상 4:4 게임만 진행되었다고 가정한다.
...
blue
설명:
첫 번째 경기는 아이템전이고 3번 플레이어가 제일 먼저 들어왔으므로
red팀이 승리한다.

두 번째 경기는 스피드전이고, 3->7->1->4->5->6->8->2 번 순서대로
골인하였지만 6, 8, 2번 유저는 10초 이상 차이가 나서 리타이어로
점수를 받을 수 없다. red 팀은 10 + 4 = 14 점이고,
blue 팀은 8 + 6 + 5 = 19 점을 획득하여 blue팀이 승리한다.

채점 방식
입력 케이스들은 다음과 같은 종류로 구별되며, 한 종류의 케이스를
다 맞추어야 그 종류에 배정된 점수를 받을 수 있다.
...
승리한 팀을 정하는 방법이 몇가지 방법과 우선순위로 주어져 있어 그대로 프로그램하면 답을 구할 수 있다.
"""
...
#    print("allteam = ", allteam)
    ...
    if mode == ('item'):
        ...
        for cnt in range(4):
            scoreTable = {1:10.1, 2:8, 3:6, 4:5, 5:4, 6:3, 7:2, 8:1}
            for i in range(8):
