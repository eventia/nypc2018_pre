...
해설
주어진 문장의 첫 글자들 만을 모아 줄임말을 만들어야 한다. 한글을 처리하는 것이 어려울 수 있으나 여러가지 라이브러리를 사용할 수 있다.
*/
...
int main(){
	int countNumber;
	char sString[65], longString[16*65];
...
for i in range(countNumber):
	longString = input()
#	longString = str(longString,"utf-8")
	longString = longString.split()
	for j in range(len(longString)):
		lines = lines + longString[j][0]
	lines = lines + '\n'
